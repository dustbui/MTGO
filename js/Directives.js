(function (angular) {
        ...
        return {
            ...
            link: function (scope, elem, attrs) {
                ...
                angular.element(elem).on("paste", function (e) {
                    ...
                    setTimeout(function () {
                        if (scope.value.length >= limit) scope.value = value;
                    }, 5);
                })
            }
        }
    ...
    app.directive('gameCard', [function() {
        return {
            restrict: "E",
            scope: { card: '=', useAbility: '=' , getManaIcons: '='},
            templateUrl: "html/partials/Card.html",
        }
    }]);
    
    app.directive('gameCardDeckEdit', [function() {
        return {
            restrict: "E",
            scope: { card: '=', getManaIcons: '='},
            templateUrl: "html/partials/DeckEditCard.html",
        }
    }]);
    
    app.directive('gameCardMin', [function() {
        return {
            restrict: "E",
            scope: { card: '=', viewCard: '=' , getManaIcons: '=' },
            templateUrl: "html/partials/CardMin.html",
        }
    }]);
    
    app.directive('battlefield', ['$window', function($window) {
        return function (scope, element, attrs) {
            element.width(element.parent()[0].offsetWidth - 180);
            
            angular.element($window).bind('resize', function sidebarResize() {
                element.width(element.parent()[0].offsetWidth - 180); // 20 is browser padding i guess?
                scope.$digest();
            });
        }
    }]);

    app.directive('gameSidebar', ['$window', function ($window) {
        return function (scope, element, attrs) {
            element.height(element.height() - 64);
            
            angular.element($window).bind('resize', function sidebarResize() {
                element.height($window.innerHeight - 64 - 10); // 20 is browser padding i guess?
                scope.$digest();
            });
        }
    }]);
